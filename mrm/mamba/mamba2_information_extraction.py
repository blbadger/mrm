...
import transformers
from transformers import AutoTokenizer, Mamba2Config, Mamba2Model, Mamba2ForCausalLM 
...
class UnrolledAutoencodingMamba(nn.Module):
    ...
    def forward(self, input_ids, labels=None, attention_mask=None):
        ...
        x = self.encoder(x, use_cache=True).last_hidden_state
        ...
        for i in range(self.tokenized_length):
            index = i % self.dim
            sliding_window = encoder_embedding[..., index:index+self.dim//2]
            if index+self.dim//2 > self.dim:
                residual = index+self.dim//2 - self.dim
...
class TruncatedModel(nn.Module):
    # truncates the lm_head off a pretrained mamba model
...
encoder_model = Mamba2ForCausalLM(config)
decoder_model = Mamba2Model(config)

# load encoder
safetensors.torch.load_model(encoder_model, f'{checkpoint_root}/mamba_training/fineweb_mamba_512_s256_n16_c512_b32x2/checkpoint-200000/model.safetensors')
...
output_dir = f'{checkpoint_root}/fineweb_mamba2_secondrun_information\
_{dim}\
_s{state_size}\
_n{n_layers}\
