...
def tokenize_input(example):
    example['input_ids'] = tokenizer.encode(example['text'], return_tensors='pt', truncation=True, padding='max_length', max_length=1024, padding_side='right')
    return example
...
if __name__ == "__main__":
   ...
   # count_parameters(model)
    ...
    output_dir = f"{checkpoint_root}/gsm8k_pcontrol_srm_h{n_heads}_mixed_decay_nonparallel_projs_{dim}_n{layers}_c512_b{batch_size}x{n_gpus}"
 
    dataset = load_dataset("openai/gsm8k", "main")
    train_dataset, test_dataset = dataset['test'], dataset['test'] # positive control
    train_dataset = train_dataset.map(prepare_nshot, num_proc=16).remove_columns(['prompt', 'completion'])
    test_dataset = test_dataset.map(prepare_nshot, num_proc=16).remove_columns(['prompt', 'completion'])
 
    train_dataset = train_dataset.map(tokenize_input, num_proc=16)
    test_dataset = test_dataset.map(tokenize_input, num_proc=16)
