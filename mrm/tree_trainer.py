...
@torch.no_grad()
def tree_selection_evaluation(policy_model, reward_model, test_dataset, tokenizer, device, num_eval_items=20, samples_per_item=512, batch_size=16, k=50, random_selection=False):
	"""
	...
	for i in tqdm(range(num_eval_items)):
		...
		if not random_selection:
			...
			selected_samples = generated_ids[:k, :]
...
def tree_expansion_evaluation(
	...
	for i in tqdm(range(num_eval_items)):
		...
		while current_length < context_limit:
			...
			last_rewards = output.logits
			top_indices = torch.topk(last_rewards, samples_to_keep).indices
			selected_samples = output_ids[top_indices, :]
			input_ids = selected_samples.repeat(expansion_factor, 1)
			# expand caches
			reward_model.select_and_expand_cache(top_indices, expansion_factor)
			policy_model.select_and_expand_cache(top_indices, expansion_factor)
