...
class MixedRepeatHeads(nn.Module):
    ...
    def __init__(self, dim: int, seq_len: int, hidden_dim: int, n_heads: int, expanded_convs=False, decay=False, use_projections=True):
        ...
        self.use_projections = use_projections
        if use_projections:
            self.proj_head = nn.module_v3.sequential.ModuleList(Linear(dim, hidden_dim) for i in range(n_heads))
            self.out_proj = Linear(dim, dim)
        ...
        self.mixer_heads = nn.module_v3.sequential.ModuleList(ColRepeatCausalLinear(seq_len, embedding_dim=hidden_dim, decay=decay, decay_constant=seq_len//512) for i in range(n_heads//2)) \
                         + nn.module_v3.sequential.ModuleList(RowRepeatCausalLinear(seq_len, embedding_dim=hidden_dim, decay=decay, decay_constant=seq_len//512) for i in range(n_heads//2))
...
class RepeatHeads(nn.Module):
    ...
    def __init__(self, dim, seq_len, hidden_dim, n_heads, expanded_convs=False, combined_heads=False, use_projections=True, decay=False):
        ...
        if combined_heads:
            ...
            self.mixer_heads = nn.module_v3.sequential.ModuleList(ColRepeatCausalLinear(seq_len, embedding_dim=hidden_dim, decay=decay, decay_constant=seq_len//512) for i in range(n_heads))
...
class MixerBlock(nn.Module):
    ...
    def __init__(self, hidden_dim: int, seq_len: int, expansion_factor=4, heads=None, kernel=1, expanded_convs=False, mixed_heads=False, combined_heads=False, decay=False, parallel_heads=False, use_projections=True):
        ...
        self.channel_in = Linear(hidden_dim, hidden_dim * expansion_factor)
        self.channel_out = Linear(hidden_dim * expansion_factor, hidden_dim)
...
class RecurrentSRM(nn.Module):
    ...
    def __init__(
        ...
        self.input_layer = Embedding(vocab_size, dim=hidden_dim)
        ...
        self.output_layer = Linear(hidden_dim, vocab_size, bias=False)
...
if __name__ == "__main__":
    ...
    input_tokens = input_tokens.repeat(80, 1)
