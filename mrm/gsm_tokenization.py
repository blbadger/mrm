...
def map_dataset(train_path, test_path, split_index=40000, packed=False, context=1024, minimum_length=0):
	...
	"""
	train_text = load_dataset("TinyGSM/TinyGSM", split="train", num_proc=16).skip(split_index)
	test_text = load_dataset("TinyGSM/TinyGSM", split="train", num_proc=16).take(split_index)
