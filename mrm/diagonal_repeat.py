...
class DiagonalRowCausalLinear(nn.Module):
...
class ConstDiagonalColCausalLinear(nn.Module):
...
class ConstDiagonalRowCausalLinear(nn.Module):
...
class CombinedDiagonalRepeatCausalLinear(nn.Module):
...
if __name__ == "__main__":
    ...
    output_dir = f"{checkpoint_root}/fineweb_h{n_heads}_constdiagonal_mixed_decay_k{kernel}_{dim}_n{layers}_c512_b{batch_size}x{n_gpus}"
